...
class MainFrame(ModifiedFrame):
	def __init__(self, root, *args, **kwargs):
		...
		self.create_frame1(
			...
			spinbox_kwargs = dict(
				width = 46,
				foreground = "green",
				command = self.update_spin_boxes_range_of_selection,
				),
			)

		self.frame1.scenes_listbox.bind(
			"<<ListboxSelect>>",
		...
		self.add_by_default(
			...
			obj_kwargs = dict(
				...
				width = 15,
				height = 2,
				command = self.render_video_function,
				state = "disabled",
				),
			add_kwargs = dict(
				row = 4,
		...
		self.render_video_button.bind(
			"<Button-1>",
			self.rendering_status_update,
			)

	def create_frame1(self, padx, pady, frame_add_kwargs, button_kwargs, spinbox_kwargs):
		self.frame1 = ModifiedFrame(
			self.frame,
			text = "Select the file you want to render",
			add_method = "grid",
			add_kwargs = frame_add_kwargs,
			)

		self.frame1.add_by_default(
			"manim_dir_set_label",
			"Label",
			obj_kwargs = dict(
				text = "Enter the path of the file you want to render.",
				font = DEFAULT_FONT,
				),
			add_kwargs = dict(
				row = 0,
		...
		self.frame1.add_by_default(
			"refresh_scenes_button",
			"tk.Button",
			disable_when_required = True,
			obj_kwargs = dict(
				text = "Refresh Scenes",
				**button_kwargs,
				command = self.refresh_scenes_function,
				),
			add_kwargs = dict(
				row = 2,
				column = 3,
		...
		self.frame1.add_by_default(
			"select_all_scenes_button",
			"tk.Button",
			disable_when_required = True,
			obj_kwargs = dict(
				text = "Select All",
				**button_kwargs,
				command = self.select_all_scenes_button_function,
				),
			add_kwargs = dict(
				row = 3,
				column = 3,
		...
		self.frame1.add_by_default(
			"clear_selected_scenes_button",
			"tk.Button",
			disable_when_required = True,
			obj_kwargs = dict(
				text = "Clear Selection",
				**button_kwargs,
				command = self.clear_selected_scenes_button_function,
				),
			add_kwargs = dict(
				row = 4,
				column = 3,
		...
		self.frame1.add_by_default(
			"select_scenes_button",
			"tk.Button",
			disable_when_required = True,
			obj_kwargs = dict(
				text = "Select Scenes",
				**button_kwargs,
				command = self.select_scenes_button_function,
				state = "disabled",
				),
			add_kwargs = dict(
				row = 5,
		...
		self.frame1.add_by_default(
			"selected_scenes_label",
			"Label",
			obj_kwargs = dict(
				text = "Selected scenes :",
				font = DEFAULT_FONT,
				foreground = "green",
				),
			add_kwargs = dict(
		...
		self.frame1.add_by_default(
			"selected_scenes_in_Listbox_label",
			"Label",
			obj_kwargs = dict(
				text = self.NO_SCENE_SELECTED_MESSAGE,
				font = DEFAULT_FONT,
				foreground = "green",
				),
			add_kwargs = dict(
	...
	def create_frame2(self, padx, pady, frame_add_kwargs):
		...
		self.create_output_quality_frame(
			padx,
			pady,
			frame_add_kwargs = dict(
				row = 0, 
		...
		self.frame2.add_by_default(
			"leave_progress_bars_checkbox",
			"Checkbutton",
			disable_when_required = True,
			obj_kwargs = dict(
				text = "Leave Progress Bars",
				),
			add_kwargs = dict(
				row = 1,
				column = 1,
		...
		self.frame2.add_by_default(
			...
			obj_kwargs = dict(
				text = "Transpatent",
				command = self.transpatent_function,
				),
			add_kwargs = dict(
				row = 1,
				column = 2,
		...
		self.frame2.add_by_default(
			...
			obj_kwargs = dict(
				text = "Choose Colour",
				command = self.choose_colour_function,
				),
			add_kwargs = dict(
				row = 3,
				column = 1,
		...
		self.frame2.add_by_default(
			"clear_background_colour_entrybox_button",
			"Button",
			disable_when_required = True,
			obj_kwargs = dict(
				text = "Clear Colour",
				command = lambda : self.frame2.background_colour_entrybox.delete(0, tk.END),
				),
			add_kwargs = dict(
				row = 3,
				column = 2,
		...
		self.frame2.add_by_default(
			...
			obj_kwargs = dict(
				text = "Output Name :",
				font = DEFAULT_FONT, foreground = "green",
				),
			add_kwargs = dict(
				row = 4,
				column = 0,
		...
		self.checkbox_list = (
			self.frame2.play_video_checkbox,
			self.frame2.leave_progress_bars_checkbox,
			self.frame2.transparent_checkbox,
			)
		self.command_list = (
			["!alternate", "selected"],
			["!alternate", "selected"],
	...
	def create_output_type_frame(self, padx, pady, frame_add_kwargs):
		...
		self.frame2.output_type_frame = ModifiedFrame(
			self.frame2.frame,
			text = "Output type",
			add_method = "grid",
			add_kwargs = frame_add_kwargs,
		...
		for i, quality in enumerate(output_type_list):
			self.frame2.output_type_frame.add_by_default(
				quality.replace(
					" ",
					"_"
					).lower() + "_radiobutton", "Radiobutton",
				disable_when_required = True,
				obj_kwargs = dict(
					text = quality,
					variable = self.frame2.output_type_frame.radiobutton_variable,
					value = quality.lower().replace(" ", "_"),
					),
				add_kwargs = dict(
					row = i,
					column = 0,
	...
	def create_output_quality_frame(self, padx, pady, frame_add_kwargs, entry_width):
		...
		def edit_when_default_quality_in_video_quality():
			...
			for item in (
				self.frame2.output_quality_frame.height_entrybox,
				self.frame2.output_quality_frame.width_entrybox,
				):
				if selected_button == output_quality_list[-1]:
					item["state"] = "normal"
					item.insert(
						0, ["480" if item is self.frame2.output_quality_frame.height_entrybox else "854"][0]
						)
		...
		self.frame2.output_quality_frame = ModifiedFrame(
			self.frame2.frame,
			text = "Output quality",
			add_method = "grid",
			add_kwargs = frame_add_kwargs,
		...
		for i, quality in enumerate(output_quality_list):
			self.frame2.output_quality_frame.add_by_default(
				quality.replace(" ", "_") + "_radiobutton",
				"Radiobutton",
				disable_when_required = True,
				obj_kwargs = dict(
					text = quality.title(),
					variable = self.frame2.output_quality_frame.radiobutton_variable,
					value = quality.lower(),
					command = edit_when_default_quality_in_video_quality,
					),
				add_kwargs = dict(
					row = i % 4,
					column = i // 4,
		...
		for event in (
			...
			self.frame2.output_quality_frame.height_entrybox.bind(
				event,
				set_default_width_from_height,
				)
			self.frame2.output_quality_frame.width_entrybox.bind(
				event,
	...
	def enable_disabled_items(self):
		if self.initial_state_of_disabled_items_already_defined:
			...
			for item_state, item in zip(
				self.initial_state_of_disabled_items,
				self.frame1.list_of_objects_to_be_disabled,
				):
	...
	def browse_button_function(self):
		...
		initialdir_preference = (
			self.LAST_RENDERED_FILE_DIR,
			self.MANIM_DIR,
			os.path.expanduser("~"),
			)
		...
		req_file = filedialog.askopenfilename(
			...
			title = "Select the file you want to render.",
			filetypes = (
				(
					"Python Files",
					("*.py", "*.pyc")
	...
	def refresh_scenes_function(self):
		...
		if os.path.isfile(file_path) and (file_path[-3:] == ".py" or file_path[-4:] == ".pyc"):
	...
	def get_selected_item_in_listbox(self):
		return [
			self.frame1.scenes_listbox.get(i)
			for i in self.frame1.scenes_listbox.curselection()
			]
	...
	def transpatent_function(self):
		list_of_items = (
			self.frame2.background_colour_entrybox,
			self.frame2.background_colour_button,
			self.frame2.clear_background_colour_entrybox_button,
			)
	...
	def render_video_function(self):
		...
		if not self.frame2.transparent_checkbox.instate(["selected"]):
			render_kwargs.update(
				dict(
					bg_colour = self.frame2.background_colour_entrybox.get(),
					)
				)

		error, render_time = render_scenes(
			self.current_file,
			selected_scenes,
			*render_args,
			**render_kwargs,
