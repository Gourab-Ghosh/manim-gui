...
class Listbox(tk.Listbox):
	def __init__(self, root, *args, **kwargs):
		...
		self.scrollbar = ttk.Scrollbar(self.frame, orient = tk.VERTICAL, takefocus = 0)
...
class Frame:
	def __init__(self, root, add_method = "pack", *args, frame_type = ttk, add_kwargs = {}, **kwargs):
		self.root = root
	...
	def add(self, obj_name, obj_type, add_method, create_new_object = True, obj_args = [], obj_kwargs = {}, add_args = [], add_kwargs = {}):
	...
	def add_by_default(self, obj_name, object_type, add_method = "grid", create_new_object = True, obj_args = [], obj_kwargs = {}, add_args = [], add_kwargs = {}):
		if object_type.startswith("tk.") or object_type.startswith("ttk."):
			exec(f"obj_type = {object_type}", globals())
		else:
			try:
				exec(f"obj_type = ttk.{object_type}", globals())
			except AttributeError:
				exec(f"obj_type = tk.{object_type}", globals())
		try:
			obj = self.add(obj_name, obj_type, add_method, create_new_object, obj_args, obj_kwargs, add_args, add_kwargs)
		except TclError:
...
class EditDefaultFrame(Frame):
	def __init__(self, root, main_frame, *args, **kwargs):
		...
		padx = 25; pady = 5
		self.add_by_default("manim_dir_set_label", "Label", obj_kwargs = dict(text = "Enter the Default Manim directory path or Video directory path and click the corresponding button below to set or reset the directory path.", font = DEFAULT_FONT), add_kwargs = dict(row = 0, columnspan = 4, padx = padx, pady = pady))
		self.add_by_default("choose_file_label", "Label", obj_kwargs = dict(text = "Choose File :", font = DEFAULT_FONT, foreground = "green"), add_kwargs = dict(row = 1, column = 0, padx = padx, pady = pady, sticky = tk.E))
		self.add_by_default("dir_entrybox", "Entry", obj_kwargs = dict(foreground = "green", width = 100), add_kwargs = dict(row = 1, column = 1, columnspan = 2, padx = padx, pady = pady))
		self.add_by_default("browse_button", "Button", obj_kwargs = dict(text = "Browse Path", command = self.browse_button_function), add_kwargs = dict(row = 1, column = 3, padx = padx, pady = pady, sticky = tk.W))
		self.add_by_default("manim_set_directory_button", "Button", obj_kwargs = dict(text = "", command = self.set_manim_directory_button_function), add_kwargs = dict(row = 2, column = 1, padx = padx, pady = pady))
		self.add_by_default("video_set_directory_button", "Button", obj_kwargs = dict(text = "", command = self.set_video_directory_button_function), add_kwargs = dict(row = 2, column = 2, padx = padx, pady = pady))
		self.add_by_default("dir_set_confirmation_label", "Label", obj_kwargs = dict(text = "", font = ("halvetica", 11)), add_kwargs = dict(row = 3, columnspan = 4, padx = padx, pady = pady))

		self.check_set_directory_button_text()

	def browse_button_function(self):
		initialdir = None
		for path in (self.VIDEO_DIR, self.MANIM_DIR, os.path.expanduser("~")):
			if initialdir == None:
				initialdir = path
			else:
				break
		req_dir = filedialog.askdirectory(initialdir = initialdir, title = f"Choose the directory to reset defaults.")
		if req_dir != "":
